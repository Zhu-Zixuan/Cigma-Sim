...
def cycle_simulator_float_float(
    ...
    a_man_bits = 2 + a.fmt.mantissa_bits + a_extend_bits
    w_man_bits = 2 + wt.fmt.mantissa_bits + w_extend_bits

    a_encode, w_encode = cycle_config.a_encode, cycle_config.w_encode
    a_digit_width, w_digit_width = a_encode.digit_width, w_encode.digit_width

    a_digits = (a_man_bits + a_digit_width - 1) // a_digit_width
    w_digits = (w_man_bits + w_digit_width - 1) // w_digit_width
    ...
    if align_config.method != FloatAlignMethod.PRODUCT:
        ...
        a_nonzero = logical_right_shift(a_nonzero << a_extend_bits, a_shift)
        wt_nonzero = logical_right_shift(wt_nonzero << w_extend_bits, w_shift)
    ...
    match cycle_config.method:
        case PEProcessingStrategy.W_BIT_SERIAL:
            segment_cycles = count_cycles_bit_serial(
                data=wt_nonzero,
                digit_per_data=w_digits,
                digit_width=w_digit_width,
