...
class TernaryDigits:
    ...
    def mask_bits(self, mask: Tensor | int) -> Self:
    ...
    def shift_left(self, amount: Tensor | int) -> Self:
        """Shift all bit planes left by `amount`."""
        return self.__class__(
            nonzero=self.nonzero << amount,
            negative=self.negative << amount,
            positive=self.positive << amount,
        )

    def shift_right(self, amount: Tensor) -> Self:
        """Shift all bit planes right by `amount`."""
        return self.__class__(
            nonzero=logical_right_shift(self.nonzero, amount),
            negative=logical_right_shift(self.negative, amount),
            positive=logical_right_shift(self.positive, amount),
        )
...
class QuinaryDigits:
    ...
    def shift_right(self, amount: Tensor) -> Self:
        ...
        return self.__class__(
            ...
            negative=logical_right_shift(self.negative, amount),
            positive=logical_right_shift(self.positive, amount),
            doubled=logical_right_shift(self.doubled, amount),
        )
