...
class FloatDataWrap:
    ...
    def from_value(cls, value: Tensor, fmt: FloatFormat) -> Self:
        ...
        value_binary = value.to(fmt.value_dtype).contiguous().view(value_fmt.storage_dtype)
    ...
    def to_value(self) -> Tensor:
        ...
        if self.fmt in _DTYPE_TO_FLOAT_FORMAT.values():
            # simple way for standard floating-point type
            return self.payload.contiguous().view(self.fmt.value_dtype)

        scale = self.unbiased_exponent - self.fmt.mantissa_bits

        sign_value = 1 - 2 * self.sign_bool.to(self.fmt.dtype)

        value = self.full_mantissa * sign_value
        value = value.to(self.fmt.value_dtype).ldexp(scale)

        value = torch.where(self.is_inf, sign_value.to(self.fmt.value_dtype) * float("inf"), value)
    ...
    def truncated(self) -> Tensor:
        ...
        return self.payload.to(self.fmt.dtype).where(self.is_special, self.sign_field | self.fmt.max_unsigned)
...
def parts_to_float_binary(
    ...
    *,
    ...
    def _find_msb_position(x: Tensor) -> Tensor:
    ...
    mantissa = mantissa.to(fmt.dtype)
    exponent = exponent.to(fmt.dtype)

    to_zero = mantissa == 0

    msb_position = _find_msb_position(mantissa)

    biased_exp = exponent + msb_position + fmt.exponent_bias
    ...
    to_zero = to_zero | (rshift >= bit_width)

    align_drop_shift = rshift.clamp(0, bit_width - 1)

    round_offset = round_offset_true_form(mantissa, sign_bool, rshift.clamp(0, bit_width - 1), rounding)

    aligned_mantissa = (mantissa << (-rshift).clamp_min(0)) >> align_drop_shift
    ...
    mantissa_field = aligned_mantissa & fmt.mantissa_mask
