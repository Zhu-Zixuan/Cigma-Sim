...
_align_config = ExponentAlignConfig(
    method=FloatAlignMethod.PRODUCT,
)

_summation_frac = -1
_rounding_mode = RoundingMode.FULL_DOWN
...
def cigma_value_float_float(
    ...
    w_fmt: FloatFormat,
    *,
    product_extend_bits: int,
    trailing_op: ElementWiseOp = lambda x: x,
) -> Tensor:
    """Simulate CIGMA floating-point GEMM output."""
    product_frac = a_fmt.mantissa_bits + w_fmt.mantissa_bits + product_extend_bits

    value_config = InternalPrecisionConfig(
        prod_frac=product_frac,
        summ_frac=_summation_frac,
        rounding_mode=_rounding_mode,
    )

    return value_simulator_float_float(
        a=FloatDataWrap.from_value(a, a_fmt),
        wt=FloatDataWrap.from_value(wt, w_fmt),
        align_config=_align_config,
        value_config=value_config,
        trailing_op=trailing_op,
    )
...
class _DataConfig:
    ...
    window_size: int
    lane_count: int
    product_extend_bits: int = 0


_data_configs: dict[tuple[FloatFormat | int, FloatFormat | int], _DataConfig] = {
    (8, 8): _DataConfig(segment_len=64, window_size=4 * 8, lane_count=8),
    (16, 16): _DataConfig(segment_len=64, window_size=8 * 4, lane_count=16),
    (BFLOAT16, BFLOAT16): _DataConfig(segment_len=64, window_size=5 * 4, lane_count=16, product_extend_bits=18),
    (FLOAT16, FLOAT16): _DataConfig(segment_len=64, window_size=6 * 4, lane_count=16, product_extend_bits=13),
    (FLOAT32, FLOAT32): _DataConfig(segment_len=64, window_size=13 * 2, lane_count=32, product_extend_bits=7),
}
...
def cigma_cycle(
    ...
    *,
    ...
    if isinstance(a_fmt, QuantFormat) and isinstance(w_fmt, QuantFormat):
        ...
        a, wt = validate_and_prepare(a, wt, cfg.segment_len)
    ...
    *,
    ...
    if isinstance(a_fmt, FloatFormat) and isinstance(w_fmt, FloatFormat):
        ...
        return cigma_value_float_float(
            ...
            wt,
            w_fmt,
            product_extend_bits=cfg.product_extend_bits,
            trailing_op=trailing_op,
        )
