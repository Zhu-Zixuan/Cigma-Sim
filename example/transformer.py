...
"""Shared runner for Transformer profiling experiments."""
...
from .presets import FLOAT_ARCH_NAMES, get_float_cycle_fn, get_float_value_fn
...
def run_transformer_experiment(
    ...
    prepare_inputs: Callable[[Any, Any, torch.device], dict[str, Tensor]],
    *,
    chunk_configs: dict[str, ChunkConfig] | None = None,
) -> None:
    """Run one Transformer profiling experiment from CLI arguments."""
    # --- parse args ---

    args = parse_args(f"{model_name} profiling", FLOAT_ARCH_NAMES)
    ...
    profiler = Profiler(
        arch=arch,
        cycle_fn=get_float_cycle_fn(arch) if sim_cycle else None,
        value_fn=get_float_value_fn(arch) if sim_value else None,
    ...
    with torch.no_grad(), redirect_stderr_to_file(str(stderr_file)):
        for i in tqdm(
            ...
            model_inputs = prepare_inputs(batch, tokenizer, device)
            model(**model_inputs)
